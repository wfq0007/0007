#include <iostream>
using namespace std;

#define u8 unsigned char
#define u32 unsigned long
u32 RK[32] = { 0 };//轮密钥

//一些函数
void enc_sm4(bool pad, int size_p, u8* input, u8* output);//加密
void dec_sm4(int len, u8* input, u8* output);//解密
void rk_gen(u8* key);//轮密钥生成
void u8_32(u8* in, u32* out);//字节转字
void T_key(u8 i, u32 a, u32 b, u32 c, u32 d);//密钥生成的T函数
u32 T_plain(u8 i, u32 a, u32 b, u32 c, u32 d);//轮加密/轮解密的T函数
void printt_u8(int len, u8* input);

//一些固定参数
/******************************定义系统参数FK的取值****************************************/
const u32 FK[4] = {
	0xa3b1bac6,
	0x56aa3350,
	0x677d9197,
	0xb27022dc
};

/******************************定义固定参数CK的取值****************************************/
const u32 CK[32] = {

	0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
	0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
	0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,
	0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
	0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,
	0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
	0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,
	0x10171e25,0x2c333a41,0x484f565d,0x646b7279
};

/******************************SBox参数列表****************************************/
const u8 S_BOX[256] = {

	
};

void u8_32(u8* in, u32* out) {
	
	for (u8 i = 0; i < 4; i++) {
		u32 temp = 0;
		for (u8 j = 0; j < 4; j++) {
			temp = temp ^ ((u32)in[4 * i + j] << (24 - 8 * j));
		}
		out[i] = temp;
	}
}
void T_key(u8 i, u32 a, u32 b, u32 c, u32 d) {
	u32 temp;
	temp = a ^ b ^ c ^ CK[i];
	//过函数T
	//32——8,过S盒
	u8 a0, a1, a2, a3;
	a0 = S_BOX[(u8)(temp >> 24)];
	a1 = S_BOX[(u8)(temp >> 16)];
	a2 = S_BOX[(u8)(temp >> 8)];
	a3 = S_BOX[(u8)(temp)];
	u32 B, B13, B23;
	B = (u32)a0 << 24 ^ (u32)a1 << 16 ^ (u32)a2 << 8 ^ (u32)a3;
	B13 = (B << 13) ^ (B >> 19);
	B23 = (B << 23) ^ (B >> 9);
	RK[i] = B ^ B13 ^ B23 ^ d;
}
void rk_gen(u8* key) {
	u8 i;
	u32 K[4] = { 0 };
	u8_32(key, K);//密钥由字节转为字
	for (i = 0; i < 4; i++) {
		K[i] = K[i] ^ FK[i];//与系统参数异或
	}
	T_key(0, K[1], K[2], K[3], K[0]);
	T_key(1, K[2], K[3], RK[0], K[1]);
	T_key(2, K[3], RK[0], RK[1], K[2]);
	T_key(3, RK[0], RK[1], RK[2], K[3]);
	for (i = 4; i < 32; i++) {
		T_key(i, RK[i - 3], RK[i - 2], RK[i - 1], RK[i - 4]);
	}
}
u32 T_plain(u8 i, u32 a, u32 b, u32 c, u32 d) {
	u32 temp;
	temp = a ^ b ^ c ^ RK[i];
	//过函数T
	//32——8,过S盒
	u8 a0, a1, a2, a3;
	a0 = S_BOX[(u8)(temp >> 24)];
	a1 = S_BOX[(u8)(temp >> 16)];
	a2 = S_BOX[(u8)(temp >> 8)];
	a3 = S_BOX[(u8)(temp)];
	u32 B, B2, B10, B18, B24, C;
	B = (u32)a0 << 24 ^ (u32)a1 << 16 ^ (u32)a2 << 8 ^ (u32)a3;
	B2 = (B << 2) ^ (B >> 30);
	B10 = (B << 10) ^ (B >> 22);
	B18 = (B << 18) ^ (B >> 14);
	B24 = (B << 24) ^ (B >> 8);
	C = B ^ B2 ^ B10 ^ B18 ^ B24 ^ d;
	return C;
}
void enc_sm4(bool pad, int size_p, u8* input, u8* output) {
	//要实现任意长度的明文加密
	int len = size_p / 16;
	//轮加密
	for (int num = 0; num < len; num++)//对不用补丁的组
	{
		u32 P[4] = { 0 };
		u8_32((input + num * 16), P);//明文由字节转为字
		u32 X1, X2, X3, X4;
		X1 = P[0];
		X2 = P[1];
		X3 = P[2];
		X4 = P[3];
		u32 temp;
		for (u8 i = 0; i < 32; i++) {
			temp = T_plain(i, X2, X3, X4, X1);
			X1 = X2;
			X2 = X3;
			X3 = X4;
			X4 = temp;
		}
		//最后一轮反序变换
		//合成，并变成u8
		for (u8 i = 0; i < 4; i++) {
			u8 j = 24 - 8 * i;
			output[16 * num + i] = (u8)(X4 >> j);
			output[16 * num + 4 + i] = (u8)(X3 >> j);
			output[16 * num + 8 + i] = (u8)(X2 >> j);
			output[16 * num + 12 + i] = (u8)(X1 >> j);
		}
	}
	if (pad)//最后一组
	{
		int n = size_p - len * 16;
		u8 plt[16] = { 0 };
		for (int i = 0; i < n; i++) {
			plt[i] = input[len * 16 + i];
		}
		//加密
		u32 P[4] = { 0 };
		u8_32(plt, P);//明文由字节转为字
		u32 X1, X2, X3, X4;
		X1 = P[0];
		X2 = P[1];
		X3 = P[2];
		X4 = P[3];
		u32 temp;
		for (u8 i = 0; i < 32; i++) {
			temp = T_plain(i, X2, X3, X4, X1);
			X1 = X2;
			X2 = X3;
			X3 = X4;
			X4 = temp;
		}
		//最后一轮反序变换
		//合成，并变成u8
		for (u8 i = 0; i < 4; i++) {
			u8 j = 24 - 8 * i;
			output[16 * len + i] = (u8)(X4 >> j);
			output[16 * len + 4 + i] = (u8)(X3 >> j);
			output[16 * len + 8 + i] = (u8)(X2 >> j);
			output[16 * len + 12 + i] = (u8)(X1 >> j);
		}
	}
}
void dec_sm4(int len, u8* input, u8* output) {
	//要实现任意长度的密文解密
	//轮解密
	for (int num = 0; num < len; num++)
	{
		u32 P[4] = { 0 };
		u8_32((input + num * 16), P);//密文由字节转为字
		u32 X1, X2, X3, X4;
		X1 = P[0];
		X2 = P[1];
		X3 = P[2];
		X4 = P[3];
		u32 temp;
		for (u8 i = 0; i < 32; i++) {
			temp = T_plain(31 - i, X2, X3, X4, X1);
			X1 = X2;
			X2 = X3;
			X3 = X4;
			X4 = temp;
		}
		//最后一轮反序变换
		//合成，并变成u8
		for (u8 i = 0; i < 4; i++) {
			u8 j = 24 - 8 * i;
			output[16 * num + i] = (u8)(X4 >> j);
			output[16 * num + 4 + i] = (u8)(X3 >> j);
			output[16 * num + 8 + i] = (u8)(X2 >> j);
			output[16 * num + 12 + i] = (u8)(X1 >> j);
		}
	}
}
void printt_u8(int len, u8* input) {
	for (int i = 0; i < len; i++)//16进制输出
	{
		int a = input[i];
		if (a < 16)
			cout << "0x0" << hex << a << " ";
		else
			cout << "0x" << hex << a << " ";
		if ((i + 1) % 8 == 0)
			cout << endl;
	}
	cout << endl;
}