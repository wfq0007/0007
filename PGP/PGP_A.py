...
IDa='ALICE123@YAHOO.COM'
IDb='BILL456@TAHOO.COM'
key=keyexchange_A(IDa,IDb)
#Encrypt session key：SM2 encryption
#使用B的公钥加密
c1,c2,c3=encrypt(xb,yb,key)
c=c1+c2+c3
#Encrypt data： Symmetric encryption
#使用key调用对称密码加密data
data="abcdefghsjieioeubdcusiejocodkfosjafuebc"
#这里对称密码算法使用SM4
crypt_sm4=CryptSM4()
crypt_sm4.set_key(key.encode(),SM4_ENCRYPT)
encrypt_value=crypt_sm4.crypt_ecb(data.encode())
#发送
...
try:
    ...
    s.send(c.encode())
    tp = s.recv(1024).decode()
    s.send(encrypt_value)
    tp=s.recv(1024).decode()
    if tp=='received':
        print("对方已收到")
    s.close()
